...
- `apps/` holds the externally exported applications, assembled from Client / Host mixtures.
    ...
    - `apps/desktop` is the initial Electron shell: it supervises the complete `dsh web` application over a private loopback listener. A future IPC fetch carrier can replace that transport without changing the client packages ([decision](../feature/2026-08-24-electron-desktop-shell.md)).
...
| Carrier layer | `dsh-host-webserver` | Web HTTP and upgrade: static serving + `/api/*`→handler forwarding + WebSocket upgrade route + close semantics; plugin bundle endpoint + `__DSH_BOOT__` manifest injection (fed by the web plugin registry) | Browser access and the initial Electron shell's private loopback transport; zero workspace dependencies (the registry arrives by structural injection) |
...
| IPC bridge subclass (future alternative — not used by the current shell) | an Electron shell | IPC serialization round trip | would replace the current private loopback transport by swapping only doFetch; contract and base class stay unchanged |
