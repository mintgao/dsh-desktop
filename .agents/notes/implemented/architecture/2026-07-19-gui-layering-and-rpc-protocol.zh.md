...
- `apps/` 作为对外导出的应用入口，可以由 Client / Host 混合组装。
    ...
    - `apps/desktop` 是首个 Electron 壳：它通过私有 loopback listener 监管完整 `dsh web` 应用。将来的 IPC fetch 载体可以替换该传输，而不改变客户端包（见[决策](../feature/2026-08-24-electron-desktop-shell.zh.md)）。
...
| 承载层 | `dsh-host-webserver` | Web HTTP 与 upgrade：静态服务 + `/api/*`→handler 转发 + WebSocket upgrade route + close 语义；插件 bundle 端点 + `__DSH_BOOT__` manifest（元数据清单）注入（由 web 插件注册表供给） | 浏览器访问与首个 Electron 壳的私有 loopback 传输；零 workspace 依赖（注册表经结构注入到达） |
...
| IPC 桥子类（未来备选——当前壳未使用） | Electron 壳 | IPC 序列化往返 | 只需替换 doFetch，即可取代当前私有 loopback 传输；约定与基类保持不变 |
