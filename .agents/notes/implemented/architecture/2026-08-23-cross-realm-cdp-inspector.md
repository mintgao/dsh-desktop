...
Client Console enable is a version-1 correlated handshake. The Worker allocates a branded subscription id and owns the provisional listener, timeout, and synchronously disposable handle before sending the enable frame. The Client retains the exact Runtime-session and subscription tuple, installs Console and global-error hooks before reporting success, and repeats the retained outcome without reinstalling for duplicate enables. `Runtime.enable` joins one initial enable epoch and returns only after the snapshot's Runtime backends and Console subscriptions are ready; realms opened during that epoch enter the later dynamic path. Failed installation, timeout, send failure, disconnect, disable, realm removal, DevTools close, and router disposal share idempotent cleanup. A dynamic realm failure withholds its context and closes only that connection-local realm session. Active events require the exact source generation, Runtime session, and subscription id. Each session serializes arguments into its own `console` object group, so cleanup or `Runtime.discardConsoleEntries` can release one connection without invalidating another. Context and Fiber arguments use the same semantic reference and DOM reverse mapping as evaluation results.
...
- A held Client Console acknowledgement keeps `Runtime.enable` pending; exact success permits event delivery, while failure, timeout, stale correlation, disable, disconnect, and disposal remove provisional state without announcing a context.
...
Protocol version 1 rejects version-0 peers and has no negotiation or shim. The Host, Worker, and browser Client artifacts must match; restarting only the Inspector does not update a Client bundle already loaded by a running page.
