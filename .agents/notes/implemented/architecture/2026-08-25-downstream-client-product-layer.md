...
The task-completion notification is the first explicit application of this rule. `@deepseek-ai/dsh-client-ui-session-notifications` observes the public Session Controller list and UI Session pending-interaction sources, registers its own General settings row, and uses the renderer's Web Notifications API. Electron maps that standard API to macOS Notification Center, so its main process receives no task lifecycle branch or renderer bridge. The public application supplies the Developer ID-signed identity that Electron 43 requires for this mapping; [signed public desktop releases](../process/2026-08-25-signed-public-desktop-releases.md) owns that release obligation. The plugin stays reusable and defaults to off when a composition makes no product choice; the Mint Bundle alone mounts it with `defaultMode: background`. Host settings own that product default because browser boot rows do not project Host Cordis config; an omitted Client config starts safely off until the settings scope adopts the Host section. The generic `web` Profile does not inherit that selection.
...
Package tests cover notification transition policy, subagent aggregation, pending-interaction suppression, permission states, click navigation, settings persistence, and disposal. Config-dump coverage proves the generic `web` Profile excludes the notification row and the `desktop-mint` Profile includes it exactly once with the Mint default. A keyless browser test applies the Mint Bundle over the shipped Web tree and captures the system-notification payload after a real Agent status transition. Release acceptance installs a signed and notarized DMG and verifies background delivery plus notification activation against macOS Notification Center. Documentation and configuration gates verify the package manifests, client roster, bilingual references, and invariant companions.
