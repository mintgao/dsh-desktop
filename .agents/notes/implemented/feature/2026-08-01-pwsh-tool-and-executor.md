...
- Windows semantics differ where the platform differs: forced termination reports exit 1 with no signal, while a `kill()`-initiated background handle still stamps its authoritative `killed` lifecycle state before settlement; sandbox classification honors that state instead of treating numeric exit diagnostics as denial or runner failure. PowerShell writes CRLF, which tests normalize.
