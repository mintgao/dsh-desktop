...
The desktop and embedded DSH runtime update as one tested application unit. In explicitly activated signed mode, the [automatic upstream desktop release](../process/2026-08-27-automatic-upstream-desktop-releases.md) process observes published upstream `dsh-v*` Releases, adopts them in order, runs the required checks, maps each version to an exact desktop tag, and publishes the signed application. The [pre-certificate stage](../process/2026-08-27-pre-certificate-unsigned-desktop-previews.md) uses suffixed manual previews and cannot enter this feed. A failure stops before the adoption push and records a blocker Issue. Withdrawal removes a problematic Release from update discovery without deleting its tag or assets; installed applications are never remotely downgraded.
