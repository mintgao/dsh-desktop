...
The local targets are unsigned macOS arm64 and x64 applications named DSH Desktop with the Mint wave icon and the `io.github.mintgao.dsh-desktop` bundle identifier. They are suitable for source builds and user-level installation. [Mint desktop downstream development](../process/2026-08-24-mint-desktop-downstream-development.md) owns Developer ID signing, hardened runtime, notarization, native-architecture DMGs, and public artifact publication. Universal binaries and automatic updates are not present.
