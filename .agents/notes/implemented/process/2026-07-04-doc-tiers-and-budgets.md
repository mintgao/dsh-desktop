...
- **Authenticated managed instructions.** [verify-md-wrap](../../../../scripts/verify-md-wrap.ts) masks only the exact root [`AGENTS.md`](../../../../AGENTS.md) Vibe-managed region after strict UTF-8 decoding, manifest ownership, two equal lowercase SHA-256 values, and the installed bytes' universal-newline-normalized digest all authenticate it. The manifest, markers, digest, or ownership failing any predicate rejects the file; no other file or region is exempt. Vibe Kit owns the region and hashes, while the repository owns the checker and explicit budget. Recovery restores the installed files through the supported Vibe Kit path and confirms them with `./bin/vibe doctor . --format json`; it never edits the managed bytes or hashes locally.
- **Ceilings are an enforcement frontier that ratchets.** A doc at or below its [target](../../../../docs/AGENTS.md#wordcount-budgets) keeps at least 5% headroom as its ceiling ratchets down; a doc above target keeps a frozen ceiling that prevents growth until it reaches the target. Root `AGENTS.md` is frozen at exactly 2,858 whole-file words without headroom: 909 managed words and 1,949 project-owned words. A smaller managed block follows ordinary ratcheting; a larger block fails until a healthy Vibe Kit installation and reviewed decision establish the new explicit whole-file ceiling. When the gate goes red, relocate or condense; raise a ceiling only with explicit PR justification.
