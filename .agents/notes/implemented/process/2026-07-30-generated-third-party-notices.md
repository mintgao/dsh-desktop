...
**Tiering uses both declaring area and manifest section.** A package is a runtime dependency when the root manifest or any manifest outside `DEV_ONLY_AREAS` — `packages/test-support/`, `packages/test-support/client-runtime/`, `website/`, `examples/`, `native/` — names it under `dependencies` or `optionalDependencies`. Root runtime dependencies can ship in the root-owned desktop application, while root `devDependencies` remain development-only. Section names alone are still wrong for excluded areas: a test-support package declares `vitest` under `dependencies` without shipping it, and the root source-run scripts execute through `tsx`, which no manifest declares as a runtime dependency at all (the generator marks it runtime explicitly).
