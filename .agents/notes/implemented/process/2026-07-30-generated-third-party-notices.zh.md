...
**分层同时依据声明方所在区域与 manifest 字段。** 只要根 manifest 或 `DEV_ONLY_AREAS`——`packages/test-support/`、`packages/test-support/client-runtime/`、`website/`、`examples/`、`native/`——之外的任一 manifest 在 `dependencies` 或 `optionalDependencies` 里点名某个包，它就是运行时依赖。根运行时依赖可以随根目录持有的桌面应用交付，而根 `devDependencies` 仍属于开发依赖。对排除区域而言，单看字段名仍会出错：测试支撑包把 `vitest` 写在 `dependencies` 里却并不交付它；而根目录的源码运行脚本通过 `tsx` 执行，根本没有任何 manifest 把它声明为运行时依赖，只能由生成器显式标记。
