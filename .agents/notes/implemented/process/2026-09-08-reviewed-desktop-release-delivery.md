...
Mint uses the owner-approved [single-maintainer main review policy](../../../../docs/decisions/20260908-desktop-reviewed-delivery.md#configured-main-review-policy). Required source checks, tag protections and explicit publication approval remain mandatory. A permanent policy avoids recurring protection changes that invalidate timestamp-bound activation evidence; it gives up mandatory approval by a second person. Other distributions retain independent review by default.

Activation binds the normalized distribution configuration and the exact live-control report. A configuration change, including stricter artifact requirements, invalidates the prior binding. Refresh control observations and the reviewed binding without changing unchanged protection rules or the historical baseline. Reports retain administrator identity and required attestations; unrelated account fields remain private. A control-record-only successor may publish an unchanged qualified ancestor through a new run-bound plan.
