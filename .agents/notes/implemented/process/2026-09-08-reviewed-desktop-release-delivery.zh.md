...
启用记录绑定规范化的发行配置与精确的实时控制报告。配置变化（包括更严格的产物要求）会使原绑定失效。刷新控制观察与经过审查的绑定，同时保留未变的保护规则和历史基线。报告保留管理员身份与必需的证明；无关账户字段保持私有。仅修改控制记录的后继版本可以通过新的运行绑定计划，发布未变且已通过资格验证的祖先版本。
