...
**Remove this section at the first tagged release.** With no external consumers, rename or repackage freely and update all references. Backends reject old on-disk formats. SQLite uses monotonic `SCHEMA_VERSION`; `dsh-session` keeps `SESSION_FORMAT_VERSION` at `0` without compatibility.
...
- **Mint work follows [dsh-mint-client-feature](.agents/skills/dsh-mint-client-feature/SKILL.md).** `desktop-mint` owns plugin defaults; Electron owns lifecycle.
- **Desktop signing requires maintainer activation.** Follow the [staged release decision](.agents/notes/implemented/process/2026-08-27-pre-certificate-unsigned-desktop-previews.md).
