...
[`upstream-sync.yml`](.github/workflows/upstream-sync.yml) checks published upstream `dsh-v*` Releases twice an hour and adopts them in publication order. For each release it fetches the exact tag, merges it directly into `main`, runs the desktop tests and build, repository type check, documentation checks, and a generated-diff check, and records the adoption in [`.github/upstream-sync-state.json`](.github/upstream-sync-state.json). The default `DESKTOP_RELEASE_SIGNING_MODE=unsigned-preview` stage adds an `unsigned.1` prerelease suffix, atomically pushes the adoption commit and tag, and publishes unsigned arm64 and x64 DMGs as a GitHub Pre-release. Only an explicit maintainer-confirmed transition to `signed` restores exact version mapping, requires Apple signing and notarization Secrets, and permits stable update assets or Latest publication.
...
- Follow the [desktop application reference](apps/desktop/README.md) for local packages, unsigned small-group previews, the explicit signed-release transition, installed-client updates, and release withdrawal. Upstream-driven tags publish automatically after the checks required by the active release stage pass. A manually pushed desktop tag still creates a draft for exceptional releases.
