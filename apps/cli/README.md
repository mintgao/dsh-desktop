...
The invoking directory is the default workspace root. The `web`, `desktop-mint`, and `headless` profiles auto-initialize on first use from shipped templates; any other profile must be created through `dsh plugin`. `desktop-mint` applies the Mint product Bundle after the shared Web layer while retaining its own user patch.
...
Bundles named in `dsh.profile.bundles` resolve from the dsh installation first (`@deepseek-ai/dsh-base`, `@deepseek-ai/dsh-web-app`, `@deepseek-ai/dsh-desktop-mint`, `@deepseek-ai/dsh-headless`), then from the profile's own `node_modules`, where pnpm installs out-of-tree plugins.
