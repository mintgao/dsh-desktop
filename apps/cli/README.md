...
The invoking directory is the default workspace root. The `web`, `desktop-mint`, `headless`, `sdk`, `sdk-minimal`, and `acp` profiles auto-initialize on first use from shipped templates; any other profile must be created through `dsh plugin`. `desktop-mint` applies the Mint product Bundle after the shared Web layer while retaining its own user patch.
