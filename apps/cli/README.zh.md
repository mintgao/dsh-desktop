...
运行命令时所在的目录将作为默认 workspace 根目录。`web`、`desktop-mint`、`headless`、`sdk`、`sdk-minimal` 和 `acp` profile 在首次使用时会从随附模板自动初始化；其他任何 profile 都必须通过 `dsh plugin` 创建。`desktop-mint` 会在共享 Web 层之后应用 Mint 产品 Bundle，并保留自己的用户 patch。
