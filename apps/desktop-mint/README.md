...
The package pipeline installs the frozen official npm runtime from `runtime/package-lock.json`, verifies its tarball payloads, and adds separately packed Mint extensions. The native shell embeds the assembly receipt digest and rejects missing or changed payloads before startup. `runtime/assembly-input.json` identifies the supported upstream version and integrity; changing local upstream source does not change the packaged runtime. Platform-excluded optional dependencies may be absent only when the frozen dependency graph proves exclusion and the installed inventory agrees. Every installed package still receives complete tarball payload verification, including optional descendants retained by npm. Required or selected missing packages reject assembly; [platform selection](../../docs/decisions/20260913-desktop-versioned-assembly.md#frozen-platform-selection) defines the checks.
...
[`src/backend.ts`](src/backend.ts) owns readiness parsing and bounded process shutdown. [`src/navigation.ts`](src/navigation.ts) is the pure URL policy. [`src/updates.ts`](src/updates.ts) owns signed update decisions, while [`src/electron-updates.ts`](src/electron-updates.ts) adapts the signed transport. [`src/manual-updates.ts`](src/manual-updates.ts) owns prerelease reminders, [`src/github-releases.ts`](src/github-releases.ts) validates the public Release API, and [`src/manual-update-preferences.ts`](src/manual-update-preferences.ts) stores those choices atomically. [`src/main.ts`](src/main.ts) selects the channel from the application version and owns native presentation. [`../../scripts/prepare-desktop-backend.ts`](../../scripts/prepare-desktop-backend.ts) stages the verified official runtime and separately packed Mint extensions; [`../../scripts/merge-desktop-update-metadata.ts`](../../scripts/merge-desktop-update-metadata.ts) validates and combines signed per-architecture metadata; [`electron-builder.yml`](electron-builder.yml) owns the macOS bundle layout and public feed identity. Run `pnpm run test:desktop` for the focused desktop tests. After `pnpm run desktop:stage`, run `pnpm run test:desktop:assembly` to exercise the actual installed Host and Client in synthetic data roots. On macOS, `pnpm run test:desktop:packaged -- "/path/to/DSH Desktop.app/Contents/MacOS/DSH Desktop"` checks the compiled bootstrap and rejects CLI overrides with a bounded exit; it uses isolated native paths. These scripts use the Node version selected by `.node-version`.
...
The [hosted GUI capability probe](../../docs/decisions/20260913-forward-native-capability.md) checks normal launch, rendered window content and application-menu quit before update qualification. Its diagnostic result cannot qualify or publish a release.
