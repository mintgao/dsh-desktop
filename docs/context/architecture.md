...
The [versioned assembly decision](../decisions/20260913-desktop-versioned-assembly.md) governs the RC.2 candidate: the native shell verifies an official frozen npm runtime plus separately packed Mint plugins and initializes its custom profile through public DSH APIs. The committed delivery source-lock remains the published baseline until normal adoption finalization; the work item retains the unfinalized target proposal.
