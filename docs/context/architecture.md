...
Mint native code lives in `apps/desktop-mint`; upstream `apps/desktop` and `apps/desktop-host` retain separate shell and runtime ownership. Mint uses the ordinary `desktop-mint` CLI Profile rather than the reserved upstream `desktop` profile. The [Accepted integration decision](../decisions/20260910-desktop-mint-target-integration.md) and [migration registry](../decisions/20260910-desktop-mint-migration-registry.md) bind target `b2e3b2a0125854567a4a5fcba75782e42fe84901`; source integration does not establish packaged upgrade qualification. Released session generations use upstream frozen migration stages and preserve predecessors; restore requires a complete backup and the delivered baseline runtime.

The [versioned assembly decision](../decisions/20260913-desktop-versioned-assembly.md) governs the RC.2 candidate: the native shell verifies an official frozen npm runtime plus separately packed Mint plugins and initializes its custom profile through public DSH APIs. The committed delivery source-lock records the finalized RC.2 catch-up and adoption seed; the [RC.2 release work item](../work-items/20260913-mint-rc2-release/verification.md) owns the published unsigned arm64 evidence, and installed-user acceptance stays in machine-private records.
