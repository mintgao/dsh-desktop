...
A substantially different interface or complex extra capability follows the same ownership rules: presentation belongs to Client plugins or a separate interface consuming public APIs, agent capabilities belong to independently disposable plugins, and operating-system integration belongs to the native shell. Plugin-based design does not promise every future extension point already exists. Review missing public APIs before implementing a consumer; retain any unavoidable upstream modification with its contribution or removal plan. Official and Mint component versions are independent in both adoption preparation and artifact qualification.
