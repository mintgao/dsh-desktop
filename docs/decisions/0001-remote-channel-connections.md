...
Sections 1, 8, 10 and 12 were revised on 2026-09-20 for the account-ownership work item [`20260920-wechat-account-ownership`](../work-items/20260920-wechat-account-ownership/brief.md): one composition serves several WeChat accounts, each owning its identity and its durable state; the login surface a scan needs lives on a plugin-local service rather than on the seam or a registration; and the platform's one-account-one-session rule, the observations that verify it, and the recovery semantics around it are stated.
...
Account surface. A provider cannot own the login that creates it, because no registration exists before a scan, and one registration cannot hold several accounts. `packages/channel/channel-weixin` therefore also declares a plugin-local service, `ctx.channelWeixin`, which owns the account set, the login sequence, and the registrations; the accounts it creates register into `ctx.channels` like any other provider, so the Consumer keeps one enumeration point and this surface is not a second seam. The distinguishing rule: `ctx.channelWeixin` is optional, belongs to the provider package, and is read only by that platform's own setup surface — the controller and the page that drive its scan. The Consumer and the registry projection never read it, and a composition without this package is complete without it. Alternatives: a seam-level login surface on `ctx.channels` would give every platform a login contract the Feishu provider does not share, since its setup is a console walkthrough with credentials; and driving the QR sequence from the controller would put platform protocol into a product surface. Both rejected.
...
`channel-weixin` implements the documented iLink HTTP/JSON API with Node builtins: `fetch` or `node:https` for QR login, `get_qrcode_status`, `get_updates`, and `send_message`; `node:crypto` for the AES-128-ECB media path; the poll cursor in the binding record. It declares no third-party runtime dependency, because `verifyMintPackages` resolves every declared dependency against the frozen official closure and would fail assembly. The official `@tencent-weixin/openclaw-weixin` package and the MIT `cc-weixin` project are protocol references, not dependencies. Chunking, platform length limits, and the throttle policy live in the provider. A long-poll timeout the platform suggests for the next request is honoured only up to the configured bound, so the poll's worst-case cycle stays computable from the composition alone.

Login entry and one account's single session, verified against the live platform on 2026-09-20: `ilink/bot/get_bot_qrcode?bot_type=3` is the only accepted bot type — every value from `0` to `9` but `3` answers `ret=2, invalid bot_type` — and the QR it returns points at one liteapp entry, so every client implementing this API scans into the same entry. A confirmed scan returns two identities: the bot it created as `ilink_bot_id`, and the WeChat account that scanned as `ilink_user_id`. A WeChat account therefore holds one bot session at a time: a login for an account replaces the session its previous client held, the displaced client's polls and sends both fail with `errcode -14`, and it needs a scan of its own to return. The WeChat conversation itself is unchanged, so nothing in the platform's own interface tells a user that the connected client changed.
...
Token lock: the platform accepts one polling client per token and a second poller silently splits messages, so the provider takes a lock before polling and reports `unavailable` with a diagnostic when another instance holds it. Medium: an OS-level lock file created with an exclusive-create flag in the desktop data directory, keyed by the account's platform identity (`weixin-<slug>.lock`), so one machine's several accounts never contend for one file; the holder writes its instance id and a heartbeat refreshed on every poll cycle into the file, and creation fails while the file exists. Scope: cross-process for every instance on one machine, since a real file is readable across processes, and not cross-machine, which is why the page states that one account serves one connected client at a time. Stale-lock rule: a lock is stale only when its heartbeat is older than the worst-case cycle the configured bounds allow — the poll attempts multiplied by the poll timeout plus the backoffs between them — so a holder that is retrying inside its own configured bounds never looks dead to a second instance. A holder that finds a different holder id in the lock file stops polling and reports the superseded state; a crashed holder's heartbeat stops, the lock goes stale under the same threshold, and the next provider overrides it after one diagnostic log line and writes its own holder id.

Expired platform session: WeChat `errcode -14` means the platform session is gone; the provider stops polling and reports the state that requires a new scan, and the page offers it. A failure that arrives after this account completed a poll cycle while holding this record is reported as a session taken away while DSH was connected; one that arrives before that is reported as a stored session that was already unusable when DSH started, which a takeover elsewhere may have caused without being reported as one. A send that fails with the same code drives the same state and diagnostic. The two readings share the one recovery action. No retry policy, re-scan, or reconnect loop attempts to resurrect an expired session, and nothing takes an account back on its own.

Accounts. One composition serves several WeChat accounts. An account's identity is the WeChat account rather than the bot a scan mints: a confirmed scan returns the bot as `ilink_bot_id` and the scanning account as `ilink_user_id`, and only the second is stable across scans, so the registration identity is `weixin:<slug>` derived from `ilink_user_id` — lowered, with every character outside `[a-z0-9]` replaced by `-`. That slug also names the account's credential record `channel-weixin/account-<slug>` and its token-lock file `weixin-<slug>.lock` in the configured directory. Each record carries the raw user identity beside the grant, because the lowering is lossy: an account whose slug collides with another account's while their raw identities differ is refused with a diagnostic rather than overwriting the existing record. Each account keeps its own poll cursor and its own bindings, senders, and deliveries, which the Consumer keys by that identity. Startup enumerates the stored records through `CredentialProvider.listRecords()`, admits only this package's scope and the `account-<slug>` id shape, and restores every account it can read; a record inside that scope whose payload cannot be parsed registers as `unavailable` with the unreadable-login diagnostic rather than being dropped silently, and a key from the replaced single-identity layout is not an account, is not admitted, and is left untouched. Registering an account whose slug another account already holds is refused with a diagnostic when their raw identities differ, and refused as well when the stored payload is unreadable and the comparison cannot be made, which is the fail-closed reading. A re-scan of an already-registered account is an ordered handover rather than a second registration: stop that account's poll, release its lock, dispose its registration, and register a new provider carrying the new bot identity as its display name, because the registry refuses a second provider under one identity. Disconnecting one account stops its poll, releases its lock, deletes its credential record, and unregisters its provider, leaving every other account untouched; it does not release the platform session, because the platform releases that only when another login replaces it.
...
| `channel/channel-weixin` | one account's platform connection, its credential record, its token-lock file, its poll cursor, chunking and throttling | each account's registration disposer; aborts that account's poll, releases its lock; a crash leaves the lock stale, which the next instance overrides by the stale-lock rule |
