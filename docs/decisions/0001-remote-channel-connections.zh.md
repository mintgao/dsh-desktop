...
第 1、8、10、12 节于 2026-09-20 针对账号归属工作项 [`20260920-wechat-account-ownership`](../work-items/20260920-wechat-account-ownership/brief.zh.md) 修订：一个组合服务多个微信号，各自拥有其身份与持久状态；扫码所需的登录面位于插件内服务，而非 seam 或某个注册之上；并言明平台「一个账号一条会话」的规则、验证该规则的观察，以及围绕它的恢复语义。
...
账号面：提供方无法拥有创造它自己的那次登录，因为扫码之前并不存在注册，而一个注册也装不下多个账号。因此 `packages/channel/channel-weixin` 另行声明一个插件内服务 `ctx.channelWeixin`，由它拥有账号集合、登录序列与各项注册；它创建的账号像其它提供方一样注册进 `ctx.channels`，因此消费方仍只有一个枚举点，这个面也不是第二个 seam。区分规则：`ctx.channelWeixin` 是可选的、属于该提供方包，且只被该平台自己的设置面读取——即驱动其扫码的控制器与页面。消费方与注册表投影绝不读它，缺少本包的组合也不因它而残缺。替代方案：在 `ctx.channels` 上做 seam 级登录面，会让每个平台都背负飞书提供方并不共享的登录约定（飞书的设置是带凭据的控制台引导）；由控制器驱动扫码序列，则会把平台协议塞进产品面。两者均拒绝。
...
`channel-weixin` 使用 Node 内置能力实现已文档化的 iLink HTTP/JSON API：`fetch` 或 `node:https` 负责扫码登录、`get_qrcode_status`、`get_updates` 与 `send_message`；`node:crypto` 负责 AES-128-ECB 媒体路径；轮询游标存放在绑定记录中。它不声明任何第三方运行时依赖，因为 `verifyMintPackages` 会针对冻结的官方闭包解析每个声明的依赖，从而让装配失败。官方 `@tencent-weixin/openclaw-weixin` 包与 MIT 许可的 `cc-weixin` 项目是协议参考，不是依赖。分片、平台长度限制与限流策略属于提供方。平台为下一次请求建议的长轮询超时只在所配置边界之内被采纳，因此轮询的最坏周期单凭组合即可算出。

登录入口与单账号的单会话，于 2026-09-20 对照线上平台验证：`ilink/bot/get_bot_qrcode?bot_type=3` 是唯一被接受的 bot 类型——`0` 到 `9` 中除 `3` 以外的每个取值都返回 `ret=2, invalid bot_type`——而它返回的二维码指向同一个 liteapp 入口，因此实现该 API 的每个客户端都扫进同一入口。确认后的扫码返回两个身份：它创建的机器人为 `ilink_bot_id`，扫码的微信号为 `ilink_user_id`。一个微信号于是同一时刻只持有一条机器人会话：对某账号的登录会替换它前一个客户端持有的会话，被顶掉的客户端其轮询与发送都报 `errcode -14`，且需要自行扫码才能回来。微信会话本身保持不变，因此平台自身的界面没有任何东西告诉用户所连接的客户端已经更换。
...
令牌锁：平台每个令牌只接受一个轮询客户端，而第二个轮询者会静默分流消息，因此提供方在轮询之前获取一个锁，并在另一实例持有它时报告 `unavailable` 并给出诊断。介质：一个以独占创建标志在桌面数据目录中创建的 OS 级锁文件，按该账号的平台身份命名（`weixin-<slug>.lock`），因此同一台机器上的多个账号永不争用同一个文件；持有者把实例 id 与每个轮询周期刷新的心跳写入该文件，而文件存在时创建失败。范围：对同一台机器上的每个实例是跨进程的（真实文件可跨进程读取），但不是跨机器的——这正是页面说明"一个账号同一时刻只服务一个已连接客户端"的原因。陈旧锁规则：只有当心跳早于所配置边界允许的最坏周期时，锁才算陈旧——轮询尝试次数乘以轮询超时，加上其间各次退避——因此一个正在自身配置边界内重试的持有者绝不会对第二个实例表现为已死。持有者若在锁文件中发现不同的持有者 id，就停止轮询并报告被取代状态；崩溃的持有者心跳停止，锁在同一阈值下变陈旧，下一个提供方在写下一行诊断日志后覆盖它并写入自己的持有者 id。

平台会话过期：微信 `errcode -14` 意味着平台会话已失效；提供方停止轮询并报告需要重新扫码的状态，页面提供该操作。在账号于持有这条记录期间完成了一个轮询周期之后才出现的失败，报告为 DSH 连接期间被夺走的会话；在此之前出现的失败，报告为 DSH 启动时已不可用的已存会话——它很可能由别处的接管造成，但不作为接管来报告。以同一错误码失败的发送会驱动同一个状态与同一条诊断。这两种读数共用同一个恢复动作。没有任何重试策略、重新扫码或重连循环尝试复活已过期的会话，也没有任何东西自行夺回账号。

账号：一个组合服务多个微信号。账号的身份是微信号本身，而不是一次扫码铸造的机器人：确认后的扫码把机器人返回为 `ilink_bot_id`、把扫码账号返回为 `ilink_user_id`，而只有后者在多次扫码之间稳定，因此注册身份是由 `ilink_user_id` 派生的 `weixin:<slug>`——转为小写，并把 `[a-z0-9]` 之外的每个字符替换为 `-`。该 slug 同时命名该账号的凭据记录 `channel-weixin/account-<slug>` 与其令牌锁文件 `weixin-<slug>.lock`。每条记录在登录产物之外保留原始用户身份，因为这种归约是有损的：若某账号的 slug 与另一账号的相同而原始身份不同，则拒绝注册并给出诊断，而不是覆盖既有记录。每个账号保留自己的轮询游标，以及消费方按该身份为键的绑定、发送者与投递记录。启动时通过 `CredentialProvider.listRecords()` 枚举已存记录，只接纳本包的 scope 与 `account-<slug>` 的 id 形态，并恢复每个能读到的账号；属于该 scope 内、载荷无法解析的记录会注册为 `unavailable` 并给出「已存登录不可读」的诊断，而不是被静默丢弃；来自被替换的单身份布局的键不是账号，不予接纳且原样保留。注册一个 slug 已被另一账号占用的账号时，若两者原始身份不同则拒绝并给出诊断；若已存载荷不可读、无法比较，同样拒绝——这是失败关闭的读法。对已注册账号的重新扫码是一次有序交接而非第二次注册：停止该账号的轮询、释放它的锁、释放它的注册，并注册一个以新机器人身份作为展示名称的新提供方，因为注册表会拒绝同一身份下的第二个提供方。断开其中一个账号会停止它的轮询、释放它的锁、删除它的凭据记录并注销其提供方，且不触碰其它账号；它不释放平台会话，因为平台只在另一次登录替换它时才释放。
...
| `channel/channel-weixin` | 单个账号的平台连接、其凭据记录、其令牌锁文件、其轮询游标、分片与限流 | 每个账号的注册释放函数；中止该账号的轮询、归还其锁；崩溃会留下陈旧锁，由下一个实例按陈旧锁规则覆盖 |
