...
The Mint native adapter moves to `apps/desktop-mint`, retaining `io.github.mintgao.dsh-desktop`, branding, browser identity and manual updates. Upstream `apps/desktop` and `apps/desktop-host` remain separate. Sessions use `$DSH_HOME/sessions`; JSON storage uses `$DSH_HOME/storages`; query SQLite configures `:memory:` with `openAt: never` and does not open an FTS index; generic SQLite is not selected. Qualification-only overrides select a deterministic keyless provider and private paths; they do not replace persistence, settings, credentials, workspace, attachments, projection or native implementations. Third-party plugins, custom backend substitutions and arbitrary overrides are outside this bounded composition.
...
| `packages/session-query/session-query-sqlite/src/index.ts`, `packages/session-query/session-query-sqlite/src/schema.ts`; `packages/storage/storage-sqlite/src/schema.ts` | query-rebuild: exact reads, lists and filters reconstruct from authoritative restored logs, including after restart; both full-text search APIs reject with `SESSION_QUERY_SEARCH_DISABLED` under `openAt: never`, without claiming an FTS index rebuild. storage-selection: JSON and no durable query database or new SQLite backend. |
