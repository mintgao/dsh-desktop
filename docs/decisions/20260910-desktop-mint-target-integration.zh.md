...
规范清单定义完整有限组合、存储所有者和强制设置场景。保留设置 YAML/JSON 与无关键、凭据 v1 记录及合成引用、工作区 v2 状态、匿名身份、上传索引、用户编写 preset、图像对象、浏览器状态和 Mint 偏好。投影缓存由 4 变为 7：兼容旧提示可用于列表，权威折叠状态则重建。查询 SQLite 配置 schema 8、`:memory:` 和 `openAt: never`：精确读取、列表及筛选从权威日志重建，不打开 FTS 索引。两个全文搜索 API 均以 `SESSION_QUERY_SEARCH_DISABLED` 拒绝；不选择通用 SQLite schema 1。附件文件对象及别名需要重启、冲突和中断发布证据。主目录环境文件可提供代理变量，项目文件不可。这些是场景要求，不声称语法未变就证明消费者行为未变。
