...
生产恢复于 2026-09-01 在 live evidence 暴露两个 control-plane 缺陷后重新打开本决策。Observer 比较动态 workflow run name，而不是稳定 workflow path，因此成功 validation run 产生的 Observer run 全部 job 都被跳过。与此同时，必要的 protected-workflow hotfix 在 sequence-one target 写入受保护状态前使初始 signed policy receipt 失效。monotonic renewal 正确拒绝 protected policy 为 null 时的 sequence two，从而形成 bootstrap deadlock。以下修订仍待评审；另一位 Tech Lead 批准准确持久化的边界前，实施保持 blocked。

独立 Tech Lead `policy_bootstrap_decision_review` 于 2026-09-01 批准准确持久化的 Observer-path 与 sequence-one bootstrap 修订。评审验证了稳定 path routing 加权威 run recheck、任何 Finalizer credential 之前的 authorization proof、从准确历史 authorization tree 执行、一次性 null-state 与 monotonic-chain 约束、field-limited compare-and-swap state mutation、current-workflow drift 继续阻塞，以及 sequence-plus-one recovery path。评审还确认 authorization commit `add9ca24e580306e0fa67faf7bff2483a8f9cbaf` 已包含所需 verifier command，且成功 historical preflight tree 没有改变相关 verifier、workflow 或 dependency input。
...
## Observer 身份与初始 policy bootstrap 修订

workflow-run display data 不是 control-plane identity。Observer job routing 对 `.github/workflows/upstream-adoption-validation.yml` 与 `.github/workflows/desktop-release.yml` 匹配稳定 workflow path、repository、event 与 conclusion。进入 job 后再通过 GitHub API 获取权威 run，并在信任 attempt-context artifact 前重新检查 repository ID、workflow path、event 与 conclusion。path 缺失或不匹配时 fail closed；实现绝不回退到 workflow name 或 display title。

只有受保护状态 ref 已存在、schema 合法且 `policy: null` 时，才允许一次人工触发、Finalizer 所有的 bootstrap 初始化 protected policy state。它复用 `mint-finalizer` environment、Finalizer App 和现有 finalizer concurrency group。在暴露 Finalizer credential 前，当前受保护 wrapper 证明 activation 与 sequence-one receipt 来自同一个同仓库、owner-merged squash authorization，并且准确三文件 diff 只有 activation、receipt 与 signature。authorization merge SHA 必须从 GitHub 派生，不接受 operator 提供的 commit。

bootstrap checkout 该准确 authorization commit，并从历史 tree 运行 verifier，对当前 byte-identical activation、receipt 与 signature 进行验证。历史 verifier 重新检查 signature、owner 与 repository identity、authorization PR 与 commit shape、receipt expiry、Finalizer App installation 与 permission、live ruleset、sequence one 签署的 protected workflow digest，以及仍为 null 的 protected policy。receipt 必须是 sequence one 且没有 predecessor。该过程只产生一个 protected policy target，不证明之后的 workflow revision。

bootstrap state successor 只允许改变 `policy`，把 `revision` 准确加一，并设置 `updatedAt`、`updatedBy` 与 `updateRunId`。所有 cursor、delivery、attempt、candidate、validation、failure 与 publication field 必须保持 canonical identical。写入从准确 fetched state-ref commit 开始，只创建一个 successor，并使用普通 non-force push。远端移动属于 compare-and-swap failure，不产生部分写入，也不重放 stale verification result。bootstrap 不能修改 `main`、candidate ref、tag、Release、Issue 或 dispatch 另一个 workflow。policy 一旦非 null，bootstrap 永久不可用，且 protected policy 不能通过该路径回退为 null。

bootstrap 不豁免 drift。如果当前 protected workflow 与 sequence-one receipt 不同，不可逆操作仍被 `policy-drift` 阻塞，直到既有 owner-authenticated squash protocol 安装 sequence-plus-one receipt。拒绝从 null state 接受 sequence two、重新签署或替换 sequence one、手工编辑 state、忽略 workflow digest，以及回滚必要 hotfix 的恢复路径。

独立验收必须证明：动态 run name 仍按正确 path routing，而错误 path 被忽略；权威 run recheck 会拒绝 mismatch；bootstrap 会拒绝 non-null state、sequence two、predecessor、expiry、byte 或 signature mismatch、无效 authorization shape、App 或 ruleset drift 以及 compare-and-swap conflict；成功 state commit 具有准确 parent 与 field-only diff；第二次 bootstrap 不创建 commit；旧 policy 仍报告当前 workflow drift；之后正常 sequence-plus-one renewal 能通过 verify-only 与 finalization，且此前没有提前修改 source、tag、Release、Issue、candidate 或 cursor。
