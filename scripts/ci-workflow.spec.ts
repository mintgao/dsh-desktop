...
const officialRepositoryPullRequest = `${officialRepository} && github.event_name == 'pull_request'`
...
describe('CI workflow', () => {
  ...
  it('keeps required Wine and split native Windows jobs with failover, plus a master-only standby', () => {
    ...
    expect(windows.if).toBe(officialRepositoryPullRequest)
    ...
    for (const [jobName, job] of [['windows-build', windowsBuild], ['windows-coverage', windowsCoverage], ['windows-native-tests', windowsNativeTests], ['windows-observational', windowsObservational]] as const) {
      ...
      expect(job.if).toBe(officialRepositoryPullRequest)
    }
  ...
  it('requires release-shaped Python runtime validation on every published target', () => {
    ...
    expect(pythonRuntime).toMatchObject({
      if: officialRepositoryPullRequest,
...
describe('Issue lifecycle workflow', () => {
  it('runs the lifecycle job on every official-repository PR/review event but gates token and board steps', () => {
    const lifecycle = loadWorkflow('.github/workflows/issue-lifecycle.yml')
    const policy = loadWorkflow('.github/workflows/issue-policy.yml')
    const lifecycleJob = workflowJob(lifecycle, 'lifecycle')
    if (!Array.isArray(lifecycleJob.steps)) throw new TypeError('Issue lifecycle job must define steps')

    // The downstream guard prevents this official-repository workflow from
    // minting a Project token here. In the official repository it runs on every
    // subscribed event; write-capable steps remain gated by review state.
