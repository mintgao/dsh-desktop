...
import { digest, hex, object, readJson, release, sameRelease, sourceLock, string, textField } from './evidence.ts'
 ...
 */
export async function adoptionPlan(config: DeliveryConfig, root: string, lockPath: string, observations: unknown, base: string, version: string, api: GitHub, kind = 'upstream', prior?: Record<string, unknown>): Promise<Record<string, unknown>> {
  hex(base, 40)
  ...
  if (!['upstream', 'desktop', 'replacement'].includes(kind)) throw new Error('Unknown release kind')
  if (kind !== 'upstream' && (prior === undefined || !sameRelease(release(prior.upstream), lock.release))) throw new Error('Same-upstream preparation requires exact delivered evidence')
  const next = kind === 'upstream' ? release(discovery.next) : lock.release
...
export async function prepareAdoption(config: DeliveryConfig,
  ...
  if (!existsSync(checkout)) {
    ...
    const merged = plan.releaseKind === 'upstream' ? attempt('git', ['-c', 'core.hooksPath=/dev/null', 'merge', '--no-ff', '--no-commit', upstream.commit], { cwd: checkout, env: environment }) : { status: 0 }
  ...
  const proposedLock = { schemaVersion: 2, upstreamRepository: config.upstreamRepository, release: upstream, predecessor: plan.releaseKind === 'upstream' ? previous.release : previous.predecessor, observed: plan.releaseKind === 'upstream' ? plan.observations : previous.observed, adoptionSeed: { commit: seed, tree } }
...
export async function applyAdoption(config: DeliveryConfig,
  ...
  if (lock.schemaVersion !== 2 || lock.adoptionSeed?.commit !== seed || lock.adoptionSeed.tree !== tree || !sameRelease(lock.release, release(plan.upstream))) throw new Error('Seed lock identity mismatch')
  ...
  if (bootstrapSeedLockDigest === undefined) {
    ...
    deliveryPredecessor(sameRelease(previous.release, lock.release) ? 'desktop' : 'upstream', lock, { upstream: previous.release })
  }
  ...
  if (head === seed) {
    ...
    if (object(object(await api.request('GET', headPath)).object).sha !== seed) throw new Error('Remote movement before branch update')
...
export function verifyFinalization(root: string, config: DeliveryConfig, commit: string): ReturnType<typeof sourceLock> {
  ...
  if (lock.schemaVersion !== 2 || lock.adoptionSeed === undefined) throw new Error('Production qualification requires schema2 bot finalization')
