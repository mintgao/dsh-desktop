...
it('keeps local observation inputs out of CI and shell interpolation, before exact environment approval', () => {
  const qualify = workflow('desktop-delivery-qualify')
  const native = object(object(qualify.jobs).native)
  const runs = (native.steps as unknown[]).map(object).map(step => typeof step.run === 'string' ? step.run : '').join('\n')
  expect(runs).toContain('compatibilityKind')
  expect(runs).toContain('if [ "$mode" = forward ]; then exit 0; fi')
  expect(readFileSync('.github/workflows/desktop-delivery-qualify.yml', 'utf8')).toContain('result_name=build-receipt.json')
  const mutate = workflow('desktop-delivery-mutate')
  const steps = (object(object(mutate.jobs).plan).steps as unknown[]).map(object)
  const assembly = steps.findIndex(step => String(step.run).includes(' assemble-forward '))
  const plan = steps.findIndex(step => String(step.run).includes(' promotion-plan '))
  expect(assembly).toBeGreaterThanOrEqual(0)
  expect(plan).toBeGreaterThan(assembly)
  expect(object(steps[assembly]!.env).FORWARD_OBSERVATION_BASE64).toBe('${{ inputs.local_observation_base64 }}')
  expect(object(steps[assembly]!.env).FORWARD_OBSERVATION_SHA256).toBe('${{ inputs.local_observation_sha256 }}')
  for (const step of steps) expect(String(step.run)).not.toContain('${{ inputs.local_observation')
  expect(object(object(mutate.jobs).mutate).environment).toBe('mint-publication')
})
